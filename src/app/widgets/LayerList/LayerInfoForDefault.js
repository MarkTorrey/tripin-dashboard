...
], function(declare, array, lang, graphicsUtils, aspect, LayerInfo, gfx, domConstruct, domAttr, webMercatorUtils,  Deferred, DeferredList, jsonUtils) {
      ...
      if( this.layerObject && (!operLayer.subLayer || operLayer.subLayers.length === 0)) {
        if (layer.renderer) {
          if (layer.renderer.infos) {
            legendInfos = layer.renderer.infos; // todo
          } else {
            legendInfos.push({
