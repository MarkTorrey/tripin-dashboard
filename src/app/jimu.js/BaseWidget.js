...
define([
  ...
  var clazz = declare([_WidgetBase, _TemplatedMixin], {
    ...
    constructor: function(){
      ...
      this.topicHandles.push(topic.subscribe('publishData', lang.hitch(this, this._onReceiveData)));
      this.topicHandles.push(topic.subscribe('dataFetched', lang.hitch(this, this._onReceiveData)));
      this.topicHandles.push(topic.subscribe('noData', lang.hitch(this, this._onNoData)));
    },
    ...
    destroy: function(){
      var i = 0;
      for(i = 0; i < this.topicHandles.length; i++){
        this.topicHandles[i].remove();
      }
  ...
  //hasLocale: Boolean
  //    If undefined, the framework will try to check whether the i18n files exists by convention
  //    If true, the framework will load the i18n files by convention
  //    If false, the framework will not load the i18n files even if it exists.
  clazz.hasLocale = undefined;
  //hasStyle: Boolean
  clazz.hasStyle = undefined;
  //hasConfig: Boolean
  clazz.hasConfig = undefined;
  //hasUIFile: Boolean
  clazz.hasUIFile = undefined;

  /********* these properties are used by builder**********/
  //hasSettingPage: Boolean
  clazz.hasSettingPage = undefined;
  //hasSettingUIFile: Boolean
  clazz.hasSettingUIFile = undefined;
  //hasSettingLocale: Boolean
  clazz.hasSettingLocale = undefined;
  //hasSettingStyle: Boolean
  clazz.hasSettingStyle = undefined;

  //inPanel: Boolean
  //    the widget is displayed in panel by default, developer can change this behavior.
  //    If it's not in panel, the widget will be displayed directly
  clazz.inPanel = true;
