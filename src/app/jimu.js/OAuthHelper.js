...
define(
  ...
  function(lang, dojoJson, Url, html, on, cookie, Deferred, ioquery, idManager) {
      ...
      signIn: function() {
        ...
        if (this.popup) {
          ...
          var left = 0,
            ...
            selfWidth = 600,
            selfHeight = 500;
          left = (window.innerWidth - selfWidth) / 2;
          top = (window.innerHeight - selfHeight) / 2;

          var features = "width=" + selfWidth + ",height=" + selfHeight + ",location=yes,status=yes,scrollbars=yes";
          var popWin = window.open(authUrl, "esriOAuth", features);
          if (popWin && popWin.closed || !popWin) {
            alert("The login window is blocked by the browser. Please let the browser unblock it, then refresh the site.");
          }else{
